...
function getSkillIcon(name: string, logoKey?: string) {
  const key = (logoKey || name).toLowerCase().replace(/[^a-z0-9]/g, '');
  const src = skillLogoMap[key];
  if (src) {
    return <img src={src} alt={name} className="w-full h-full object-contain rounded-md" loading="lazy" />;
  }
  return <CubeIcon />;
}
...
export function SkillsSection({ currentTheme, isDarkMode }: SkillsSectionProps) {
  ...
  const allSkills = resumeData.skills.webDev.flatMap((cat) =>
    cat.skills.map((skill) => ({ ...skill, groupCategory: cat.category }))
  );
    ...
    <section id="skills" className="py-12 sm:py-16 border-t border-white/8 relative scroll-mt-16" style={{ fontFamily: appleFontStack }}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="mb-6 overflow-x-auto pb-1 -mx-1 px-1"
          ...
          >
            ...
            {filteredSkills.length === 0 ? (
              ...
              >
                ...
                <p className="text-sm text-white/35 font-mono">No technologies in this category yet.</p>
              </motion.div>
            ) : (
              <motion.div
                layout
